...
### snapshot
快照保留的集群某个时间点的快照，同CopyTable/Export等相比，snapshot速度非常快。若是同集群拷贝，使用`clone_snapshot`进行拷贝，若是不同集群，可以使用`ExportSnapshot`进行拷贝。如下。
...
若需要导出到其他HBase集群，可在shell中执行命令`sudo -u hbase hbase org.apache.hadoop.hbase.snapshot.ExportSnapshot -snapshot <snap_name> -copy-to hdfs://<namenode>:8020/hbase -mappers 16`，snapshot拷贝的数据不在`/hbase/data/default`目录，而在`/hbase/archive/data/default/`目录。
...
配置见[HBase-0.94.4+bulkload 海量数据载库神器 ](http://chxt6896.github.io/hbase/2013/06/06/hbase-bulkload.html)

报错如下：
```
16/12/16 18:35:10 INFO zookeeper.ClientCnxn: Opening socket connection to server localhost/127.0.0.1:2181. Will not attempt to authenticate using SASL (unknown error)
16/12/16 18:35:10 WARN zookeeper.ClientCnxn: Session 0x0 for server null, unexpected error, closing socket connection and attempting reconnect
...
可通过`-c`指定zookeeper地址，如下：
```
~~$ hadoop jar hbase-VERSION.jar completebulkload [-c /path/to/hbase/config/hbase-site.xml] /user/todd/myoutput mytable~~
$ hadoop jar hbase-VERSION.jar completebulkload -Dhbase.zookeeper.quorum=<ZK_HOST> /user/todd/myoutput mytable
```
